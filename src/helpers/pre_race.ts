...
export const preRaceCheck = async (): Promise<boolean> => {

    const logger = getLogger('PRE_RACE');
    ...
    if (SETTINGS.PAUSE_RATIO !== -1) {
        ...
        const toPause = uploading.filter(torrent => {
            ...
            if (torrent.ratio < SETTINGS.PAUSE_RATIO) {
                logger.debug(`Ratio for ${torrent.name} is below threshold. Not pausing.`);
                return false;
            }

            //Ratio is pausable. Check category next
            if (SETTINGS.PAUSE_SKIP_CATEGORIES.includes(torrent.category)) {
                logger.debug(`Category ${torrent.category} for ${torrent.name} is in the skip list. Will not pause!`);
                return false;
            }
            ...
            if (SETTINGS.PAUSE_SKIP_TAGS.some(skipTag => torrentTags.includes(skipTag))) {
                logger.debug(`Torrent ${torrent.name} has tags (${torrent.tags}) in common with skip tags. Will not pause!`);
                return false;
            }
