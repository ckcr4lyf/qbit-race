...
const resume = async (logPrefix, torrents) => {
    ...
    for (let x = 0; x < torrents.length; x++) {
        ...
        if (torrent.state === 'downloading') {
            logger_1.feedLogger.log(logPrefix, `We are still downloading, not resuming rest.`);
        ...
        if (torrent.state === 'stalledDL' && (torrent.added_on * 1000) > reannounceYoungest) {
            logger_1.feedLogger.log(logPrefix, `There is a torrent in reannounce phase, not resuming rest.`);
            return; //This torrent is also still in the reannounce phase
        }
    }
    const paused = torrents.filter(torrent => torrent.state === 'pausedUP');
    if (paused.length === 0) {
        logger_1.feedLogger.log(logPrefix, `No downloading, nothing to resume either.`);
        return;
    }
    logger_1.feedLogger.log(logPrefix, `No downloading torrents. Resuming ${paused.length} torrents...`);
    try {
        await api_1.resumeTorrents(paused);
    }
    catch (error) {
        logger_1.feedLogger.log(logPrefix, `Failed to resume torrents.`);
        process.exit(1);
    }
